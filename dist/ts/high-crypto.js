...
export async function generateHybridKeypair() {
    try {
        // Type assertion needed if tsc cannot infer the correct type from imported module
 ...
 */
export async function encryptString(plaintext, recipientPubKeyHex) {
    try {
        const recipientPubKey = hexToBytes(recipientPubKeyHex);
        // Ensure ml_kem768_x25519 is correctly typed or accessed
        const cryptoInstance = ml_kem768_x25519; // Temporary type assertion if types are missing/incorrect
        const { sharedSecret, kemCiphertext } = await cryptoInstance.encapsulate(recipientPubKey);
 ...
 */
export async function decryptString(encrypted, privateKeyHex, kemCiphertextHex) {
    try {
        const privateKey = hexToBytes(privateKeyHex);
        const kemCiphertext = hexToBytes(kemCiphertextHex);
        // Ensure ml_kem768_x25519 is correctly typed or accessed
        const cryptoInstance = ml_kem768_x25519; // Temporary type assertion
        const sharedSecret = await cryptoInstance.decapsulate(kemCiphertext, privateKey);
        const derivedKey = hkdf(sha512, sharedSecret, undefined, HKDF_INFO, 32);
