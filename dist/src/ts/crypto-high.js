...
data, recipientPublicKeyHex) {
    // 1. Perform hybrid KEM encapsulation to get a shared secret
    const recipientPublicKey = fromHex(recipientPublicKeyHex);
    const { sharedSecret: kemSharedSecret, cipherText: kemCiphertext } = ml_kem768_x25519.encapsulate(recipientPublicKey);
 ...
 * @param recipientPublicKeyHex The recipient's public key (hex string).
 */
export async function fileEncrypt(inputPath, outputPath, recipientPublicKeyHex) {
 ...
 * @param privateKeyHex The recipient's private key (hex string).
 */
export async function fileDecrypt(inputPath, outputPath, privateKeyHex) {
