 ...
 */
export async function generateSigningKeypair(level = 'mldsa65') {
 ...
 */
export async function signMessage(message, privateKeyHex, level = 'mldsa65') {
    const dsa = getMLDSA(level);
 ...
 */
export async function verifySignature(signatureHex, message, publicKeyHex, level = 'mldsa65') {
    const dsa = getMLDSA(level);
 ...
 */
export async function encryptFileStream(inputPath, outputPath, recipientPubKeyHex) {
    const recipientPubKey = fromHex(recipientPubKeyHex);
    const { sharedSecret, cipherText } = ml_kem768_x25519.encapsulate(recipientPubKey);
    const derivedKey = hkdf(sha512, sharedSecret, undefined, HKDF_INFO, KEY_SIZE);
    const iv = randomBytes(IV_SIZE);
...
export async function decryptFileStream(inputPath, outputPath, privateKeyHex) {
